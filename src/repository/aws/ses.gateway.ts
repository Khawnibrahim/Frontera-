...
export class SesGateway implements IAwsSesGateway {
   ...
   * Requires SES_FROM_EMAIL verified in the same region as AWS_REGION.
   */
  async sendEmail(params: SendEmailParams): Promise<{ messageId: string }> {
    ...
    this.logger.log(`SES sendEmail messageId=${messageId} to=${params.to.join(',')}`);
    return { messageId };
  }
  ...
  private requireFromAddress(): string {
    ...
    if (!from) {
      throw new Error('SES_FROM_EMAIL is required to send email');
    }
    return from;
  }
}
